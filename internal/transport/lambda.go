...
func LambdaHandler(api catalog.API) func(context.Context, events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	return func(ctx context.Context, event events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
		...
		result := api.Handle(ctx, catalog.Request{Method: event.RequestContext.HTTP.Method, Path: event.RawPath, ContentType: contentType, Subject: subject, Query: event.QueryStringParameters, Body: body})
