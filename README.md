...
Changing from email sign-in to username sign-in replaces the Cognito user pool because AWS doesn't allow that setting to be changed in place. The template retains the old pool, but its users aren't migrated automatically. After deployment, use the new `UserPoolId` and `UserPoolClientId` stack outputs and create new username-based accounts.
