...
The stack creates an email-based Cognito user pool and a public app client with **no client secret**, suitable for the desktop app. Register users using Cognito `SignUp`, confirm their email with `ConfirmSignUp`, and sign in using `USER_SRP_AUTH` or `USER_PASSWORD_AUTH` through a Cognito SDK. The returned **AccessToken** is sent as `Authorization: Bearer <token>` on submissions and votes. Refresh with `REFRESH_TOKEN_AUTH`; access tokens last one hour. Keep refresh tokens in OS-protected credential storage.

API Gateway verifies the issuer, client audience, expiry, and `aws.cognito.signin.user.admin` scope. Lambda additionally requires `token_use=access` and a nonempty `sub` claim. ID tokens and caller-provided identity headers are not accepted. The Cognito methods above issue the required access-token scope. Cognito email delivery initially uses AWS's default service; configure SES if the deployment needs higher email volume.
